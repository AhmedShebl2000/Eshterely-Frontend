...
export default function NavRight({ isTransparent }) {
  ...
  return (
    <div className="flex items-center space-x-6">
      ...
      <div
        className="relative"
        onMouseEnter={() => isLargeScreen && setShowCart(true)}
        onMouseLeave={() => isLargeScreen && setShowCart(false)}
      >
        <Link
          to="/cart"
          className={`transition-colors duration-300 ${
        ...
        >
          <div className="relative">
            <div
              className={`${
                productArr.length > 0
                  ? "absolute -right-2 -top-2 text-center w-4 h-4 bg-amber-500 text-xs font-medium rounded-full"
                  : "hidden"
              }`}
            >
              {productArr.length}
        ...
        {showCart && (
          <div className="absolute right-0 top-0 mt-2 z-50 bg-white shadow-xl rounded-md w-100 gap-2 p-3 mb-2 overflow-auto">
            <CartProducts />
          </div>
        )}
      </div>
